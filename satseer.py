...
This downlaods the latest TLE (two-line element) file for a chosen satellite
(currently PACE) and generates a ground track for a specified number of future
...
Author:
    jgallen3 - Wed Jun 19 12:11:18 2024
"""
...
def predict_groundtrack(tle_lines, dates):
    """
    ...
    """
    satellite = EarthSatellite(tle_lines[0], tle_lines[1], 'PACE')
    ts = load.timescale()
...
def predict_footprint(tle_lines, dates, swath=50):
    ...
    for idx, begin_point in enumerate(track_points[:-1]):
        ...
        dist = distance(kilometers=swath)
        point1 = dist.destination(
            (begin_point[0], begin_point[1]), head_ccw_begin)
        point2 = dist.destination(
            (begin_point[0], begin_point[1]), head_cw_begin)
        point3 = dist.destination(
            (end_point[0], end_point[1]), head_cw_end)
        point4 = dist.destination(
...
def create_kml(dates, predicted_positions, track_points, boundary_points,
               filename="output/PACE_SPEXone_predict.kml"):
    """
    ...
    polygon_style.polystyle.color = simplekml.Color.changealphaint(
        150, simplekml.Color.red)
    polygon_style.linestyle.color = simplekml.Color.black
    polygon_style.linestyle.width = 2

    line_style = Style()
    line_style.linestyle.color = simplekml.Color.black
...
def main(satellite="PACE", num_days=3.0):
    ...
    dates = build_timestamps(start_datetime=None,
                             time_resolution=1,
